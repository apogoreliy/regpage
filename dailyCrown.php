...
// get the not filled strings for members by the key
    $res2=db_query ("SELECT * FROM `practices`
      WHERE `member_id`='$i' AND `date_practic`>DATE_ADD(CURRENT_DATE(),INTERVAL -7 DAY) AND `m_revival`=0 AND `p_pray`=0 AND  `co_pray`=0 AND `r_bible`=0 AND `r_ministry`=0 AND `evangel`=0 AND `flyers`=0 AND `contacts`=0 AND
